...
import type { LatLng, RouteInput, RouteOutput } from '@/modules/admins'
...
async function processRoute(route: RouteInput, apiKey: string | null): Promise<RouteOutput> {
  ...
  if (!apiKey) {
    return { containerId: route.containerId, orderedOrderIds: validStops.map(s => s.orderId), polyline: points, durationSec: estimateDurationSecFromPoints(points), source: 'fallback' }
  }
  try {
    const { polyline, durationSec } = await fetchOrsDirections(apiKey, points)
    const finalPolyline = polyline ?? points
    return { containerId: route.containerId, orderedOrderIds: validStops.map(s => s.orderId), polyline: finalPolyline, durationSec: durationSec ?? estimateDurationSecFromPoints(finalPolyline), source: 'ors' }
  } catch {
    return { containerId: route.containerId, orderedOrderIds: validStops.map(s => s.orderId), polyline: points, durationSec: estimateDurationSecFromPoints(points), source: 'fallback' }
  }
}
