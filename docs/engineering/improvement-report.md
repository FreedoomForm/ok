...
The dynamic admin mutation hardening slice passed the production Next.js build: compilation succeeded, 97/97 static pages were generated, and no `bcryptjs` Edge warnings or legacy Gemini fallback warnings were emitted. Unit coverage increased to 57 passing tests. The nested admin toggle-status endpoint now validates target IDs, prevents self-deactivation based on the authenticated principal rather than a user-controlled parent path segment, and attributes audit logs to the authenticated actor while preserving response fields.

The dynamic admin safe-projection hardening slice passed the production Next.js build: compilation succeeded, 97/97 static pages were generated, and no `bcryptjs` Edge warnings or legacy Gemini fallback warnings were emitted. Unit coverage increased to 58 passing tests. Nested admin deletion now validates target IDs and uses an explicit safe projection for the `{ message, admin }` response, preventing passwordHash and other internal fields from being disclosed.


## Client mutation and browser verification iteration

The dynamic `/api/admin/clients/[id]` PATCH route no longer accepts an untyped request body or builds a mass-assignment `any` object. `src/lib/admin/clients.ts` now owns bounded client IDs, validated update fields, typed Prisma update mapping, delivery-day serialization, optional password hashing, and a `safeClientSelect` projection that excludes the customer password. The DELETE path also validates the dynamic client ID. Malformed JSON and invalid payloads return controlled `400` responses, empty effective updates are rejected, group-admin scope checks remain enforced, and the successful `{ message, client }` response envelope remains intact apart from removing the previously unsafe password field.

The client seam has seven focused regression assertions covering ID bounds, empty/out-of-range values, field mapping without mass assignment, password handling, delivery-day serialization, and secret-safe projection. The complete unit suite now passes **64/64** tests, and the database-backed integration suite passes **2/2** tests.

The browser verification harness now supports an explicit `BASE_URL` for an externally managed staging server, preventing Playwright from taking ownership of a running local server. An authenticated test-admin flow asserts login, redirect to `/super-admin`, and the presence of the role dashboard logout control. The HTML reporter uses `open: never` so unattended runs terminate with a machine-readable result. Manual browser smoke successfully loaded `/`, rendered `/login`, accepted the isolated test-admin fixture, and reached `/super-admin`; a transient offline fallback was traced to the staging server process ending before navigation, not to an application authorization response. The runtime evidence is recorded separately in `docs/engineering/runtime-verification.md`.

The current quality snapshot reports **100 API routes**, **81 routes with auth lookup**, **74 with explicit role guards**, **318 explicit `any` signals**, **237 console statements**, and a **4,051-line** `AdminDashboardPage`. TypeScript, Prisma validation, lint, and whitespace checks pass with zero errors; full lint currently reports 622 warnings inherited across the wider repository, so warning reduction remains a tracked roadmap item rather than being hidden by scoped lint commands.
