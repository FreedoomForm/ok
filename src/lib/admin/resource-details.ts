...
export function isValidResourceDate(value: string): boolean {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(value)) return false
  const date = new Date(`${value}T00:00:00.000Z`)
  return !Number.isNaN(date.getTime()) && date.toISOString().slice(0, 10) === value
}

export function filterOrdersForEffectiveDate<T extends { deliveryDate: Date | null }>(rows: readonly T[], selectedDateISO: string, disabled: boolean): T[] {
  return rows.filter((row) => row.deliveryDate !== null && row.deliveryDate.toISOString().slice(0, 10) === selectedDateISO && !disabled)
}
