...
test('courier reassignment migrates future orders before disabling the source courier', async ({ page }) => {
  ...
  try {
    ...
    const sourceCheckbox = sourceRow.getByRole('checkbox', { name: /select admin|выбрать администратора/i })
...
test('universal admin edit opens selected-elements screen for multiple admins', async ({ page }) => {
  ...
  try {
    ...
    await expect(page).toHaveURL(/\/middle-admin(?:\/|$)/)
    await page.locator('[data-reference-page="admins"]').click()
    for (const name of names) await page.getByRole('checkbox', { name: `Выбрать администратора ${name}` }).check()
    const universalEdit = page.locator('[data-reference-command="edit"]')
    await expect(universalEdit).toBeEnabled()
    await universalEdit.click()
...
test('universal courier edit opens selected-elements screen for multiple couriers', async ({ page }) => {
  ...
  try {
    ...
    console.log('DIAG after 1.5s — checkboxA checked:', await page.getByRole('checkbox', { name: `Выбрать администратора ${names[0]}` }).isChecked())
...
test('universal courier edit opens and persists the selected courier', async ({ page }) => {
  ...
  try {
    ...
    await expect(page).toHaveURL(/\/middle-admin(?:\/|$)/)
    await page.locator('[data-reference-page="admins"]').click()
    await page.getByRole('checkbox', { name: `Выбрать администратора ${courierName}` }).check()
    const universalEdit = page.locator('[data-reference-command="edit"]')
    await expect(universalEdit).toBeEnabled()
    await universalEdit.click()
    const form = page.getByRole('dialog')
...
test('universal admin edit opens and persists the selected low-admin', async ({ page }) => {
  ...
  try {
    ...
    await expect(page).toHaveURL(/\/middle-admin(?:\/|$)/)
    await page.locator('[data-reference-page="admins"]').click()
    await page.getByRole('checkbox', { name: `Выбрать администратора ${adminName}` }).check()
    const universalEdit = page.locator('[data-reference-command="edit"]')
    await expect(universalEdit).toBeEnabled()
    await universalEdit.click()
    const form = page.getByRole('dialog')
