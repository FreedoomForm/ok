...
test('resource registry exposes the seventeen unified pages in the prescribed order', () => {
  assert.equal(RESOURCE_PAGE_REGISTRY.length, 17)
  assert.deepEqual(RESOURCE_PAGE_REGISTRY.map((page) => page.id), [
    'chat', 'settings', 'database', 'ingredients', 'cooking', 'dishes', 'groups', 'sets',
...
test('null permissions expose all resources and legacy warehouse/finance permissions map to child pages', () => {
  assert.equal(deriveVisibleResourcePages(null).length, 17)
