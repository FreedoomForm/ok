      ...
      <div className="flex min-h-0 flex-1 gap-3">
        ...
        <section className="min-w-0 flex-1 space-y-3 overflow-auto">
          ...
          <div data-reference-map="routes" className="relative min-h-[330px] bg-transparent p-3" onPointerDown={handleBoundaryPointerDown} onPointerUp={handleBoundaryPointerUp}>
            ...
            {(draftBoundary ?? selectedRoute?.boundary ?? null) ? <div data-reference-route-boundary className="pointer-events-none absolute z-10 bg-primary/20" style={{ left: `${(draftBoundary ?? selectedRoute?.boundary)?.x! * 100}%`, top: `${(draftBoundary ?? selectedRoute?.boundary)?.y! * 100}%`, width: `${(draftBoundary ?? selectedRoute?.boundary)?.width! * 100}%`, height: `${(draftBoundary ?? selectedRoute?.boundary)?.height! * 100}%`, border: `2px solid ${draftBoundary ? draftColor : selectedRoute?.color ?? COLORS[0]}` }} /> : null}
